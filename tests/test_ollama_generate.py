...
def _make_response(response="", done=True, thinking=None, done_reason=None, eval_count=None, model="gemma4:12B"):
...
def service() -> OllamaService:
    svc = OllamaService(base_url="http://localhost:11434", model="gemma4:12B", timeout=30)
    svc.retries = 0
    return svc
